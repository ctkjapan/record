...
function renderPicker() {
  pickerTrack.innerHTML = records.map((item, index) => `
...
function setFocusedRecord(index) {
  ...
  pickerStatus.textContent = focusedRecordIndex === selectedRecordIndex
    ? "中央のレコードをクリックして選択"
    : `${records[focusedRecordIndex].title} をクリックして変更`;
}
...
pickerTrack.addEventListener("click", (event) => {
  ...
  if (index === focusedRecordIndex) selectRecord(index);
  else {
